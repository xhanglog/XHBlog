...
<div class="x-body">
    <xblock>
        <button class="layui-btn" onclick="x_admin_show('添加用户','${pageContext.request.contextPath}/admin/showAddAdmin.html',400,400)"><i class="layui-icon"></i>添加</button>
...
<script>
    layui.use(['table','util'], function(){
        ...
        table.render({
            elem: '#I_am_a_table'
            ,id: 'I_am_a_table'
            ,url:'${pageContext.request.contextPath}/admin/user/getSysUsers'
            ,method: 'post'
            ,cellMinWidth: 80
            ,cols: [[
        ...
        form.on('switch(status)', function(obj){
            ...
            if (sysUserId == 1){
                ...
                layer.tips("系统内置账号，不能修改",obj.othis);
                layui.form.render();
                return false;
            }else{
                $.ajax({
                    type: 'POST'
                    ,url:"${pageContext.request.contextPath}/admin/user/editSwitch"
        ...
        table.on('tool(I_am_a_table)', function(obj){
            ...
            if(obj.event === 'del'){
                if (data.sysUserId == 1){
                    layer.msg("系统内置账号，不能删除!");
                }else {
                    layer.confirm('要删除吗'+data.sysUserId, function(res){
                        $.ajax({
                            url: "${pageContext.request.contextPath}/admin/user/delSysUserById",
                            type: "POST",
